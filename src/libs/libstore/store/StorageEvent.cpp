...
{
	OC_METHODGATE();
	//qDebug()<<"Entered Transaction::run() from thread "<<utility::concurrent::currentThreadID();
	StorageEventPrivate *p=p_ptr.data();
	...
	//qDebug().noquote().nospace()<<"Exiting Transaction::run() with return value "<<ret<< " from thread "<<utility::concurrent::currentThreadID();
	return ret;
}

...
#define OC_METHODGATE()  { if((reinterpret_cast<const long long unsigned int *>(this)) < reinterpret_cast<const long long unsigned int *>(OC_LOWEST_POSSIBLE_ADDRESS)){ qWarning()<<"ERROR: this < "<<OC_LOWEST_POSSIBLE_ADDRESS<<", which likely means nullptr access"; } } BUFFER_HONEYPOT

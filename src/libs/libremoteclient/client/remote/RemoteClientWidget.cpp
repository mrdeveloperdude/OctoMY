...
bool RemoteClientWidget::needConnection()
{
	OC_METHODGATE();
	QSharedPointer<Node> ctl=controller();
	QSharedPointer<Associate> na = nodeAssociate();
	if(!ctl.isNull() && !na.isNull()) {
		QSharedPointer<Settings> s=ctl->settings();
		if(!s.isNull()) {
			QString key = NODE_ONLINE_SETTINGS_KEY_BASE + na->name();
			return s->getCustomSettingBool(key, false);
		}
	}
	return false;
}
...
QSharedPointer<Associate> RemoteClientWidget::nodeAssociate()
{
	OC_METHODGATE();

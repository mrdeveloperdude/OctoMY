...
private:
	...
	JsonAsyncBackend mJsonBackend;
	SimpleDataStoreFrontend *mSimpleFrontend;
...
void SimpleDataStore::setInitialized(F *f)
{
	...
	if(nullptr!=mSimpleFrontend) {
		delete mSimpleFrontend;
		mSimpleFrontend=nullptr;
	}
	if(nullptr!=f) {
		mSimpleFrontend=OC_NEW SimpleDataStoreFrontend(*f);

	...
	if(mConfigureHelper.configure()) {
		...
		if(!mNode.isNull() && !lal.isNull()) {
			...
			if(!ass.isNull()) {
				...
				if(!connect(ui->widgetNetworkSettings, &NetworkSettingsWidget::addressChanged, this, &PairingActivity::onNetworkSettingsChange, OC_CONTYPE)) {
...
void PairingActivity::cameraPair()
{
	OC_METHODGATE();
	if(mConfigureHelper.isConfiguredAsExpected()) {

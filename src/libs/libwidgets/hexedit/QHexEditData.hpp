...
private:
	...
	public:
		...
		void updatePos(qint64 amt)
		{
			OC_METHODGATE();
			this->_pos += amt;
		}
		void updateLen(qint64 amt)
		{
			OC_METHODGATE();
			this->_len += amt;
		}
...
public:
	...
	{
	public:
		AbstractCommand(qint64 pos, QHexEditData* owner, QUndoCommand* parent = nullptr): QUndoCommand(parent), _owner(owner), _notify(true), _pos(pos) { }
	...
	{
	public:
		ModifyRangeCommand(int index, qint64 pos, const ModifyList& oldml, const ModifyList& newml, QHexEditData* owner, QUndoCommand* parent = nullptr): AbstractCommand(pos, owner, parent), _index(index), _oldml(oldml), _newml(newml)
		{
			OC_METHODGATE();
	...
	{
	public:
		InsertCommand(int index, qint64 pos, const ModifyList& oldml, const ModifyList& newml, QHexEditData* owner, bool opt, QUndoCommand* parent = nullptr): ModifyRangeCommand(index, pos, oldml, newml, owner, parent), _optimized(opt) { }
		virtual int id() const
		{
			OC_METHODGATE();
			return QHexEditData::Insert;
		}
	...
	{
	public:
		RemoveCommand(int index, qint64 pos, const ModifyList& oldml, const ModifyList& newml, QHexEditData* owner, QUndoCommand* parent = nullptr): ModifyRangeCommand(index, pos, oldml, newml, owner, parent) { }
		virtual int id() const
		{
			OC_METHODGATE();
			return QHexEditData::Remove;
		}

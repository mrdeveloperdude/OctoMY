...
void QHexEditPrivate::ensureVisible()
{
	...
	if(currline <= this->verticalSliderPosition64() || currline >= (this->verticalSliderPosition64() + vislines)) {
		this->_vscrollbar->setValue(static_cast<int>(qMax(currline - (vislines / 2), static_cast<qint64>(0))));
	}
}
...
void QHexEditPrivate::adjust()
{
	...
	if(this->_hexeditdata) {
		...
		if(totLines > visLines) {
			this->_vscrollbar->setRange(0, static_cast<int>((totLines - visLines) + 1));
			this->_vscrollbar->setSingleStep(1);
			this->_vscrollbar->setPageStep(static_cast<int>(visLines));
...
void QHexEditPrivate::internalSetCursorPos(qint64 pos, int charidx)
{
	...
	if(pos > this->_hexeditdata->length()) { /* No >= because we can add text at EOS */
		int currLine = static_cast<int>(this->_selectionstart / QHexEditPrivate::BYTES_PER_LINE);
		int lastLine = static_cast<int>(this->_hexeditdata->length() / QHexEditPrivate::BYTES_PER_LINE);
...
void QHexEditPrivate::processHexPart(int key)
{
	...
	if((this->_insmode == QHexEditPrivate::Insert) && !this->_charidx) { /* Insert a new byte */
		...
		if(this->_charidx == 1) { /* Change Byte's Low Part */
			...
			hexval = static_cast<uchar>((hexval & 0x0F) + (val << 4));
		}
...
void QHexEditPrivate::processAsciiPart(int key)
{
	...
	if(this->_insmode == QHexEditPrivate::Insert) {
		...
		this->_writer->replace(this->selectionStart(), static_cast<uchar>(key));
	}
}
...
void QHexEditPrivate::updateCursorXY(qint64 pos, int charidx)
{
	...
	if(this->_selpart == QHexEditPrivate::AddressPart || this->_selpart == QHexEditPrivate::HexPart) {
		...
		this->_cursorX = static_cast<int>(x * (3 * this->_charwidth) + this->_xposhex);
		...
		qint64 x = pos % QHexEditPrivate::BYTES_PER_LINE;
		this->_cursorX = static_cast<int>(x * this->_charwidth + this->_xposascii);
	}
}
...
void QHexEditPrivate::paintEvent(QPaintEvent* pe)
{
	...
	if(this->_hexeditdata) {
		...
		for(qint64 i = start; i <= end; i++) {
			int y = static_cast<int>((i - slidepos) * this->_charheight);
	...
	if(this->_hexeditdata && this->hasFocus() && this->_blink) {
		if(this->_insmode == QHexEditPrivate::Overwrite) {
			painter.fillRect(this->_cursorX, static_cast<int>(this->_cursorY + this->_charheight - 3), static_cast<int>(this->_charwidth), 2, this->palette().color(QPalette::WindowText));
		} else {
			painter.fillRect(this->_cursorX, static_cast<int>(this->_cursorY), 2, this->_charheight, this->palette().color(QPalette::WindowText));
		}
	}
}
...
void QHexEditPrivate::blinkCursor()
{
	...
	this->update(this->_cursorX, static_cast<int>(this->_cursorY), this->_charwidth, this->_charheight);
}

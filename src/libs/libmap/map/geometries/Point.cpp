...
void Point::draw(QPainter* painter, const QSharedPointer<MapAdapter> mapadapter, const QRect &viewport, const QPoint offset)
{
	if (!visible) {
		return;
	}
...
void Point::drawWidget(const QSharedPointer<MapAdapter> mapadapter, const QPoint offset)
{
	const QPointF c = QPointF(X, Y);
	QPoint point = mapadapter->coordinateToDisplay(c);
	point -= offset;
...
bool Point::Touches(Point* click, const QSharedPointer<MapAdapter> mapadapter)
{

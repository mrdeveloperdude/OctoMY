...
private:
	...
	QPoint mScreenMiddle; // middle of the widget (half size)

	QPoint mPreClickPixel; // used for scrolling (MouseMode Panning)
	QPoint mCurrentMousePos; // used for scrolling and dragging (MouseMode Panning/Dragging)

	QSize mSize; // size of the widget

	bool mMouseWheelEvents;
	bool mMousePressed;
	...
	QMutex mMoveMutex; // used for method moveTo()
	QPointF mTarget; // used for method moveTo()
	int mSteps; // used for method moveTo()
...
public:
	...
	//! clear all layers
	/*!
	 * Remove all layers so that none remain
	 */
	void clearLayers ();
	 ...
	 */
	void removeLayer ( QSharedPointer<Layer> layer );
	 ...
	 */
	QSharedPointer<Layer> layer ( const QString& layername ) const;
	 ...
	 *  1. Panning (default), allows the user to drag the map for navigating around.
	 *  2. Dragging, will draw a rectangule in the map while the MouseButton is pressed, and zoom to the rectangle on release
	 *  3. None, will ignore all mouse events
	 ...
	 */
	void enablePersistentCache ( const QDir& path= QDir::homePath() + "/QMapControl.cache", const int qDiskSizeMiB = 250 );
...
signals:
	...
	/*!
	 * This signal allows to receive click events within the MapWidget together with the world coordinate.

...
QPoint EmptyMapAdapter::coordinateToDisplay(const QPointF& coordinate) const
{
	...
	qreal y = (1-(log(tan(M_PI/4+deg_rad(coordinate.y())/2)) /M_PI)) /2  * (mNumberOfTiles*mTileSize);
...
QPointF EmptyMapAdapter::displayToCoordinate(const QPoint& point) const
{
	...
	qreal latitude = rad_deg(atan(sinh((1-point.y()*(2/(mNumberOfTiles*mTileSize)))*M_PI)));

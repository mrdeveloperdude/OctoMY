...
SetServoPositionsState::SetServoPositionsState(CommandParser &commandParser)
	: commandParser(commandParser)
{
	reset();
}

void SetServoPositionsState::reset()
{
...
bool SetServoPositionsState::parse(const unsigned char in)
{
	...
	if(enableByteIndex<enableByteCount) {
		...
		if(servoPositionByteIndex<4) {
			...
			servoPositionByteIndex=0;
			const float pos=converter.float32;
...
unsigned char SetServoPositionsState::enabledServoByIndex(unsigned char in)
{
	unsigned char out=0;

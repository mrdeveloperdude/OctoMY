...
#ifndef OCTO_STYLE_PRIVATE_HPP_INCLUDED
#define OCTO_STYLE_PRIVATE_HPP_INCLUDED

#include "OctoStyle.hpp"
#include "OctoStyleAnimations.hpp"

#include <QtCore/QHash>

class QStyleAnimation;

class OctoStylePrivate : public QObject
{
	Q_OBJECT
...
public:
	...
	OctoStylePrivate(OctoStyle* const style)
		: fStyle(style),
		  fAnimationFps(60)
	{
	}
	...
	OctoStyleAnimation* animation(const QObject* target) const
	{
		return fAnimations.value(target);
	}

	void startAnimation(OctoStyleAnimation* animation) const
	{
	...
	{
		OctoStyleAnimation* const animation = fAnimations.take(target);
		if (animation != NULL && animation->state() != QAbstractAnimation::Stopped)
			animation->stop();
	}

private:
	OctoStyle* const fStyle;
	int fAnimationFps;
	mutable QHash<const QObject*, OctoStyleAnimation*> fAnimations;
...
#endif // OCTO_STYLE_PRIVATE_HPP_INCLUDED

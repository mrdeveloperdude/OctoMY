...
void TestKeyStore::testBootstrap()
{
	...
	QVERIFY(!filep->exists());
	KeyStore keystore(fn, true), *keystorep=&keystore;
	QVERIFY(nullptr!=keystorep);
...
{
	const QString privKeyPEM="-----BEGIN RSA PRIVATE KEY-----\n"
	...
	const QString pubKeyPEM="-----BEGIN PUBLIC KEY-----\n"
	...
	QVERIFY(nullptr!=keystorep);
	/*
	keystorep->bootstrap(false,false);
	QVERIFY(keystorep->isReady());
	QVERIFY(!keystorep->hasError());
	QVERIFY(filep->exists());

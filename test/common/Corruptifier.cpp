...
#define randomBit() (utility::frand()<0.5)
...
{
	mSettings[(quint16)type]=qMax(val, (qreal)0.0);
}
...
QByteArray Corruptifier::corruptify(CorruptionType type, QByteArray bytes)
{
	...
	const quint8 mask=(1<<byteRemainder);
...
QByteArray Corruptifier::corruptify(QByteArray in)
{
	...
	const qreal r=(all*utility::frand());
	qreal acc=0.0;
	...
	for(qreal v:mSettings) {
		...
		type=(CorruptionType)(((quint16)type)+1);
	}

...
Status Key::put(QVariant &root, QVariant data)
{
	...
	qDebug()<<"L="<<l;
	for(auto part:w){
		qDebug()<<"STEP: "<<root.typeName()<<vtos(root);
		...
		switch(part.type){
			case(PROPERTY):{
				if(QVariant::Map == ptype){
					...
					return Status(QString("At traversal; Key type mismatch; %1 vs. %2 ").arg(QVariant::typeToName(static_cast<int>(ptype))).arg(QVariant::typeToName(QVariant::Map)));
				}
			}break;
			default:
				return Status(QString("Unknown part type '%1'").arg(KeyTokenTypeToString(part.type)));
		}
	}

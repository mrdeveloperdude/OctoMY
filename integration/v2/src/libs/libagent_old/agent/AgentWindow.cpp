	...
	, mWasEverUndelivered(false)
{
	...
	if(!mAgent.isNull()) {
		...
		if(!connect(ui->widgetDelivery, &AgentDeliveryWizard::done, this, [this]() {
		gotoNextConfigPage();
		}, OC_CONTYPE) ) {
			qWarning()<<"ERROR: Could not connect ";
		}

		if(!connect(ui->widgetPairing, &PairingWizard::done, this, [this]() {
		gotoNextConfigPage();
		}, OC_CONTYPE )) {
			qWarning()<<"ERROR: Could not connect ";
		}

		if(!connect(ui->widgetHardware, &HardwareWizard::done, this, [this]() {
		gotoNextConfigPage();
		}, OC_CONTYPE)) {
			qWarning()<<"ERROR: Could not connect ";
		}

...
//	setObjectName("DataStore");
	...
	//qDebug().noquote().nospace()<<"DataStore created () with filename="<< mFilename <<" (exists="<<exists<<") from thread "<<utility::concurrent::currentThreadID();
	if(exists) {
	...
	mCompleteFuture=QtConcurrent::run([this]() {
		OC_METHODGATE();
		//qDebug()<<"Entered DataStore::QtConcurrent::run::lambda() from thread "<<utility::concurrent::currentThreadID();
		processTransactions();
		//qDebug()<<"Exiting DataStore::QtConcurrent::run::lambda() from thread "<<utility::concurrent::currentThreadID();
	});
}
...
{
	OC_METHODGATE();
	//qDebug()<<"Entered DataStore::~DataStore() from thread "<<utility::concurrent::currentThreadID();
	// TODO: Look at how to avoid the possibility of an unecessary load during this final sync
	synchronize();
	complete();
	mCompleteFuture.waitForFinished();
	//qDebug()<<"Exiting DataStore::~DataStore() from thread "<<utility::concurrent::currentThreadID();
}
...
{
	OC_METHODGATE();
	//qDebug()<<"Entered DataStore::enqueueTransaction() from thread "<<utility::concurrent::currentThreadID();
	mTransactions.put(trans);
	//qDebug()<<"Exiting DataStore::enqueueTransaction() from thread "<<utility::concurrent::currentThreadID();
	return trans;
}
...
void DataStore::processTransactions()
{
	OC_METHODGATE();
	//QThread::msleep(2000);
	//qDebug()<<"Entered DataStore::processTransactions() from thread "<<utility::concurrent::currentThreadID();
	while(!mDone) {
	...
	//qDebug()<<"Exiting DataStore::processTransactions() from thread "<<utility::concurrent::currentThreadID();
}

...
HUDWidget::HUDWidget(QWidget *parent)
	...
	, startTime(utility::time::currentMsecsSinceEpoch<quint64>())
{
	OC_METHODGATE();
	ui->setupUi(this);
...
{
	OC_METHODGATE();
	const double cmsse=utility::time::currentMsecsSinceEpoch<quint64>()-startTime;
	...
	alpha     = static_cast<float>(   20.0 * qSin( ms /  10.0 ));
	beta      = static_cast<float>(   15.0 * qSin( ms /  10.0 ));
	roll      = static_cast<float>(  180.0 * qSin( ms /  10.0 ));
	pitch     = static_cast<float>(   90.0 * qSin( ms /  20.0 ));
	heading   = static_cast<float>(  360.0 * qSin( ms /  40.0 ));
	slipSkid  = static_cast<float>(    1.0 * qSin( ms /  10.0 ));
	turnRate  = static_cast<float>(    7.0 * qSin( ms /  10.0 ));
	devH      = static_cast<float>(    1.0 * qSin( ms /  20.0 ));
	devV      = static_cast<float>(    1.0 * qSin( ms /  20.0 ));
	airspeed  = static_cast<float>(  125.0 * qSin( ms /  40.0 ) +  125.0);
	altitude  = static_cast<float>( 9000.0 * qSin( ms /  40.0 ) + 9000.0);
	pressure  = static_cast<float>(    2.0 * qSin( ms /  20.0 ) +   30.0);
	climbRate = static_cast<float>(  650.0 * qSin( ms /  20.0 ));
	machNo    = static_cast<float>( airspeed / 650.0f);
	adf       = static_cast<float>( -360.0 * qSin( ms /  50.0 ));
	dme       = static_cast<float>(   99.0 * qSin( ms / 100.0 ));

...
private:
	...
	QPoint mScreenMiddle;
	// Scrollvalue of the offscreen image
	QPoint mScroll;
	// Scrollvalue of the zoom image
	QPoint mZoomImageScroll;

	// Widget size
	QSize mSize;
	// Size of the offscreen image
	QSize mOffScreenSize;

	QPixmap mComposedOffscreenImage;
	QPixmap mZoomImage;

	QList<QSharedPointer<Layer> > mylayers;

	// Projection-display coordinates
	QPoint mMapMiddlePixels;
	// World coordinate
	QPointF mMapMiddle;

	QPoint mWhileNewScroll;

	// Limit viewing area if desired
	QRectF mBoundingBox;
	bool mUseBoundingBox;

public:
	explicit LayerManager(MapControl*, QSize);
...
public:
	 ...
	 */
	QSharedPointer<Layer> layer(const QString&) const;
